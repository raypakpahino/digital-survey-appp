...
<div class="w-full h-full min-h-full flex-1 bg-[#f8fafc] text-slate-800 p-3 sm:p-5 font-sans box-border overflow-hidden flex flex-col justify-between select-none">
  ...
  {:else}
    ...
    {#if !activeSurveyId || !surveyTitle || questions.length === 0}
      <header class="w-full max-w-5xl mx-auto h-12 px-5 bg-white border border-slate-200 rounded-2xl flex items-center justify-between shrink-0 shadow-xs mb-3">
        ...
        <div key={currentQuestionIndex} in:fly={{ y: 15, duration: 350 }} class="w-full max-w-5xl mx-auto h-full flex flex-col justify-between overflow-hidden box-border px-2 my-auto">
          ...
          <div class="text-center space-y-1.5 shrink-0 py-2">
            ...
            {#if currentQuestion.questionImage}
              <div class="max-h-28 sm:max-h-36 max-w-md mx-auto aspect-[16/9] rounded-2xl overflow-hidden border border-slate-200 bg-white shadow-sm flex items-center justify-center p-1">
    ...
    <!-- FOOTER -->
    <footer class="w-full max-w-5xl mx-auto border-t border-slate-200 pt-2 flex items-center justify-between text-[10px] text-slate-400 font-mono tracking-wider font-semibold shrink-0">
